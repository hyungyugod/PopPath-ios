...
**Shippable:** Friendly localized share date, Korean date format, real version, the misleading static 60s badge gone, real label spacing, clean empty-cell swaps, audio that respects mute/interruption, and distinct clear/finish haptics plus a sound-toggle preview.

- **WI-8.1 · Share text, dates, version, static badge, label-gap hacks** *(I1,I2,I3,I4,I5,I7)* — Approach: format `shareText`'s raw YYYYMMDD into a friendly localized date (I1), omit the App Store/download link until the live ID exists (I2), and mark Practice shares as not recorded; `DailyChallenge.displayLabel` per-language (KO `M월 D일`), fed by WI-3.5 (I5); replace `"PopPath! v0"` with real `CFBundleShortVersionString`/build (I4); remove the static `60s`/`60초` `topBar` badge (replace with a mode label or drop — pause/clock now in HUD) (I3); replace trailing-space gap hacks in HUD DAILY/BEST labels with real HStack spacing (I7). Files: GameModel.swift, GameCore.swift, Screens.swift, GameView.swift, GameRulesTests.swift · Depends: **WI-3.5** · Risk: keep `testShareTextContainsRoundDetails` substrings; keep `DailyChallenge.id` YYYYMMDD so `testDailyChallengeSeedIsStableForDate` stays green — only `displayLabel` becomes locale-aware. Verify: update share tests; KO QA.
...
- **WI-8.3 · Audio-session robustness, distinct cues, sound-toggle preview** *(J1,J4,J5,J6)* — Approach: keep `AVAudioSession` on `.ambient` with mix-with-others so enabled SFX respects the hardware silent switch and coexists with the player's audio; add interruption/route-change/lifecycle handling via `AVAudioSession` notifications, reactivating on resume (J6); give board-clear vs round-finish distinct haptics (both `.success` today) (J4); play a short preview tone when Sound is toggled on (J5). *(J2/J3/J7 and A6 are handled in WI-3.4.)* Files: GameCore.swift, GameModel.swift, Screens.swift, PopPathApp.swift · Depends: **WI-3.4** · Risk: interruption handling mustn't crash if setup failed. Verify: mute-switch-on QA; toggle preview; simulated call interruption; clear vs finish haptics differ.
