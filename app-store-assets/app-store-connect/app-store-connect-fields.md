# PopPath App Store Connect Fields

Prepared on June 25, 2026.

## App Information

- App name: `PopPath`
- Bundle ID: `com.hyungyu.poppath`
- SKU: `poppath-ios-001`
- Version: `1.0`
- Build: `1`
- Primary category: `Games`
- Game subcategories: `Puzzle`, `Casual`
- Pricing: `Free`
- In-app purchases: `None`
- Ads: `None`
- Account/login: `None`
- Copyright: `© 2026 Hyungyu`
- Age rating target: `4+`
- Kids Category: `No`

## URLs

Replace these after the Notion pages are published publicly.

- Privacy Policy URL: `NOTION_PRIVACY_POLICY_URL_AFTER_PUBLISH`
- Support URL: `NOTION_SUPPORT_URL_AFTER_PUBLISH`
- Marketing URL: leave blank

## Korean Metadata

Subtitle:

```text
60초 화살표 체인 퍼즐
```

Promotional text:

```text
화살표가 빠져나갈 수 있는 길을 찾아 60초 안에 체인을 이어 보세요. 광고와 결제 없이 짧고 깔끔하게 즐기는 퍼즐입니다.
```

Description:

```text
PopPath는 화살표 블록의 길을 읽고 연속으로 터뜨리는 60초 퍼즐 게임입니다.

화살표가 보드 밖으로 빠져나갈 수 있는 블록만 눌러 제거하세요. 블록 하나가 사라지면 주변 길이 열리고, 빠르게 이어 누르면 체인이 쌓입니다.

클래식 모드에서는 언제든 짧게 한 판을 즐길 수 있고, 오늘의 길에서는 하루 한 번 모두 같은 보드를 풀 수 있습니다. 최고 점수, 일일 최고, 통계, 업적, 티어를 확인하며 더 깔끔한 체인을 노려 보세요.

주요 기능
- 60초 클래식 모드
- 하루 한 번 플레이하는 오늘의 길
- 최고 점수, 통계, 업적, 티어 기록
- 사운드, 진동, 움직임 줄이기, 연습용 길 표시 설정
- 광고 없음, 인앱 결제 없음, 계정 없음

짧게 한 판, 더 좋은 길, 더 높은 기록을 만들어 보세요.
```

Keywords:

```text
퍼즐,캐주얼,화살표,체인,두뇌,보드,논리,아케이드,점수,데일리
```

Release notes:

```text
PopPath 첫 출시 버전입니다.
60초 클래식 모드, 오늘의 길, 기록, 업적, 티어표를 담았습니다.
```

## English Metadata

Subtitle:

```text
Quick arrow-chain puzzle
```

Promotional text:

```text
Find open arrows, keep the chain alive, and chase a cleaner score in fast 60-second puzzle runs. No ads, no purchases.
```

Description:

```text
PopPath is a quick 60-second puzzle about reading the board and popping the right arrows in sequence.

Tap only the blocks whose arrows have a clear path out of the board. Every pop opens new space, and quick correct taps build a bigger chain.

Play a short Classic round anytime, or return for the Daily Challenge to solve the same one-shot board as everyone else. Track your best score, daily best, records, achievements, and tier progress as you improve.

Features
- Fast 60-second Classic rounds
- A once-per-day Daily Challenge
- Local records, achievements, and tier progress
- Sound, haptics, reduce motion, and practice path settings
- No ads, no in-app purchases, no account

One clean board, one better chain, one higher score.
```

Keywords:

```text
arrow,puzzle,casual,chain,daily,board,logic,arcade,score,tap
```

Release notes:

```text
Initial release of PopPath.
Includes Classic mode, Daily Challenge, records, achievements, and the tier ladder.
```

## Screenshots

Upload the PNG files from:

- `app-store-assets/screenshots-2026-06-25/iphone-6.9/`
- `app-store-assets/screenshots-2026-06-25/ipad-13/`

Use the order listed in `app-store-assets/screenshots-2026-06-25/README.md`.

## App Privacy

Use these answers in App Store Connect.

- Does this app collect data?: `No`
- Tracking: `No`
- Third-party tracking SDKs: `No`
- Data linked to the user: `No`
- Data used to track the user: `No`
- Data collected for analytics or advertising: `No`

Local-only data that stays on the device:

- Scores
- Records
- Achievements
- Settings
- Daily Challenge completion state
- Optional local notification preference

The app uses `UserDefaults` for local app state. The bundled privacy manifest declares UserDefaults required-reason API usage with reason `CA92.1`, no collected data types, and no tracking.

## Age Rating

Recommended answers:

- Cartoon or fantasy violence: `None`
- Realistic violence: `None`
- Sexual content or nudity: `None`
- Profanity or crude humor: `None`
- Alcohol, tobacco, drug references: `None`
- Medical or treatment information: `None`
- Horror/fear themes: `None`
- Simulated gambling: `None`
- Contests: `No`
- Gambling: `No`
- Loot boxes: `No`
- User-generated content: `No`
- Unrestricted web access: `No`
- Messaging or chat: `No`

Expected rating: `4+`.

## Export Compliance

- Uses encryption?: `No`
- Non-exempt encryption?: `No`
- Info.plist already contains `ITSAppUsesNonExemptEncryption = false`.

## Review Notes

```text
No login is required.
There are no ads, in-app purchases, subscriptions, tracking SDKs, or account features.
All gameplay data is stored locally on the device.
The Daily Challenge is generated locally from the current date and does not require a server.
The share button uses the standard iOS share sheet only when the user chooses to share a result.
```

## Remaining Manual Values

- Replace `NOTION_PRIVACY_POLICY_URL_AFTER_PUBLISH`.
- Replace `NOTION_SUPPORT_URL_AFTER_PUBLISH`.
- Confirm the copyright owner name if `Hyungyu` should be changed.
- Add a real support email to the Notion support/privacy pages before publishing.
